...
export const writeConfig = async (fromFile: string,toFile: string,key: string, value:any) => {
    ...
    if (fs.existsSync(fromFullFile) == false) {
        fs.writeFileSync(fromFullFile, "{}", { encoding: 'utf8' }, (err: any) => {})
    }
    ...
    fs.writeFileSync(toFullFile, JSON.stringify(data, null, 4), { encoding: 'utf8' }, (err: any) => {
        if (err) {
            console.log("writeConfig failed:",err);
        }
    })
}
